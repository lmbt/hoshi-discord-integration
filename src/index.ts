...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("before_agent_start", async (event, ctx) => {
    ...
    return {
      systemPrompt:
        ...
        "- If your action fully satisfies the request (reacting, sending a file, sending an embed) and there is nothing else to say, respond with exactly: [NO_REPLY]\n" +
        "- Do NOT narrate or confirm your actions (e.g. 'Done!', 'I reacted', 'Sent!', 'I have attached...'). The user sees the result directly.\n" +
        "- Do NOT echo emojis as text after reacting with them.\n" +
  ...
  pi.on("agent_end", async (event, ctx) => {
    ...
    // Check for sentinel indicating no reply needed
    if (finalText.trim() === "[NO_REPLY]") return;
